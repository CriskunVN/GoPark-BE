...
export const deleteSlotAndSyncZone = async (slotId) => {
    ...
    if (activeBookings.length > 0) {
        throw new AppError(`Không thể xóa slot này. Còn ${activeBookings.length} booking đang hoạt động`, 400);
    }
    // Xóa slot
...
export const getSlotsAvailable = async (startTime, endTime, parkingLotId) => {
    ...
    const availableSlots = allSlots.filter((slot) => !occupiedSlotIds.includes(slot._id.toString()));
...
export const getBookedSlotsForOwner = async (time, parkingLotId) => {
    ...
    const occupiedSlotIds = activeBookings.map((b) => b.parkingSlotId.toString());
    // 3. Trả lại slot chưa bị giao nhau
    const bookedSlots = allSlots.filter((slot) => occupiedSlotIds.includes(slot._id.toString()));
    return bookedSlots;
};
