...
export async function handleFindParking(message, session) {
    ...
    try {
        ...
        if (session.context?.location?.coordinates) {
            parkingLots = calculateDistances(parkingLots, session.context.location.coordinates);
        }
...
function calculateDistances(parkingLots, userLocation) {
    return parkingLots.map(lot => {
        if (lot.location?.coordinates) {
            const distance = calculateHaversineDistance(userLocation.lat, userLocation.lng, lot.location.coordinates[1], lot.location.coordinates[0]);
            return {
                ...lot,
                distance: distance,
...
export async function handleFindCheapParking(message, session) {
    ...
    // Xác định thành phố
    let city = "Hà Nội";
    if (messageLower.includes("đà nẵng"))
        city = "Đà Nẵng";
    if (messageLower.includes("hồ chí minh") || messageLower.includes("hcm") || messageLower.includes("sài gòn"))
        city = "Hồ Chí Minh";
    try {
...
//# sourceMappingURL=intelligentHandler.service.js.map