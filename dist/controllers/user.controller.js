...
export const updateUser = async (req, res) => {
    try {
        const { userName, email, phoneNumber } = req.body;
        const updatedUser = await User.findByIdAndUpdate(req.params.id, { userName, email, phoneNumber }, { new: true, runValidators: true }).select('-password -passwordConfirm -__v -role');
        if (!updatedUser) {
            return res.status(404).json({ error: 'Không tìm thấy user' });
        }
        res.json({
