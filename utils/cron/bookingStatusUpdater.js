...
cron.schedule('* * * * *', async () => {
  try {
    ...
    if (result.updatedCount > 0) {
      console.log(`[CRON] Updated ${result.updatedCount} bookings to 'booked' status.`);
    }
  } catch (err) {
...
const updateSlotStatusToBooked = async () => {
  ...
  for (const booking of bookings) {
    ...
    const slotIds = Array.isArray(booking.parkingSlotId) ? booking.parkingSlotId : [booking.parkingSlotId];
    for (const slotId of slotIds) {
      const res = await ParkingSlot.findByIdAndUpdate(slotId, { status: 'booked' });
      if (res) updatedCount++;
    }
