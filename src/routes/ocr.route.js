...
const recognizeLicensePlate = async (imageBuffer) => {
  try {
    ...
    if (resp.data.OCRExitCode !== 1) return "Không nhận dạng được";

    const parsedResults = resp.data.ParsedResults?.[0];
    if (!parsedResults) return "Không nhận dạng được";
    ...
    if (candidates.length===0) candidates = [{ text: normalize(parsedText), confidence: 50 }];
    ...
    for (const cand of candidates) {
      ...
      for (const p of patterns) {
        ...
        if (m) {
          let extracted = "";
          if (m.length === 3) extracted = `${m[1]}-${m[2]}`;
          else if (m.length === 4) extracted = `${m[1]}${m[2]}-${m[3]}`;
    ...
    if (!bestPlate) {
      ...
      if (m) {
        ...
        for (const letter of tryLetters) {
          ...
          if (isValidPlate(potential)) { bestPlate = potential; break; }
        }
      }
    }
...
router.post("/", upload.single("image"), async (req,res)=>{
  if(!req.file) return res.status(400).json({ success:false, error:"Chưa tải ảnh lên!" });
