...
import {
  ...
  handlePriceInfo,
  handleContactInfo,
} from './intelligentHandler.service.js';
...
async function askGeminiAIWithContext(message, session) {
  try {
    ...
    if (!GEMINI_API_KEY.startsWith('AIza')) {
      console.error(
        '❌ [DEBUG] API Key format invalid. Should start with "AIza"'
      );
    }
    ...
    if (!response.ok) {
      ...
      if (response.status === 400) {
        ...
        if (
          ...
          console.error(
            '   3. Vào Google Cloud Console > APIs & Services > Credentials'
          );
      ...
      throw new Error(
        `API Error ${response.status}: ${errorDetails.substring(0, 300)}`
      );
    }
...
export function clearSession(sessionId) {
  if (chatSessions.has(sessionId)) {
    ...
    if (session.context.pendingData.bookingId) {
      console.log(
        `💾 Lưu pending booking: ${session.context.pendingData.bookingId}`
      );
...
function extractEntities(message) {
  ...
  if (priceMatch) {
    ...
    if (priceMatch[2].includes('triệu')) value *= 1000000;
    else if (
      priceMatch[2].includes('k') ||
...
export async function processMessage(message, sessionId, userId = null) {
  ...
  try {
    ...
    if (session.context.conversationStack.length > 0) {
      const lastStep =
        session.context.conversationStack[
          session.context.conversationStack.length - 1
        ];
      const stepResponse = await handleConversationStep(
        message,
        session,
        lastStep
      );
    ...
    switch (intent) {
      ...
      case 'help':
        response = responseTemplates.text(
          `🚗 **Hướng dẫn sử dụng GoPark**\n\n1. Tìm bãi xe\n2. Chọn chỗ & giờ\n3. Thanh toán\n4. Check-in QR\n\nBạn cần hướng dẫn chi tiết phần nào?`
        );
        break;
...
async function handleBookSlotWithContext(message, session) {
  ...
  if (idMatch) parkingId = idMatch[0];
  ...
  return responseTemplates.text(
    'Bạn muốn đặt chỗ ở bãi xe nào? Hãy tìm bãi xe trước nhé!',
    {
      buttons: [
...
async function startBookingFlow(parkingId, session) {
  ...
  if (!availableSlot) {
    return responseTemplates.text(
      `😔 **${parkingLot.name} đã hết chỗ trống**\n\nVui lòng chọn bãi xe khác.`,
      {
        buttons: [
...
function buildContextString(session) {
  if (!session.context.intentHistory.length) return 'Chưa có hội thoại trước';
