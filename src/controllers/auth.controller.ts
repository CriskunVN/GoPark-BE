...
// Token payload interface used for decoded JWTs
...
export const verifyEmail = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      decoded = jwt.verify(token, process.env.JWT_SECRET) as ITokenPayload;
    } catch (err) {
      return next(
...
export const login = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!user.isActive) {
      return next(
        new AppError('Vui lòng xác nhận email trước khi đăng nhập', 401)
      );
    }

    // tạo token và gửi về cho client
    const { accessToken, refreshToken } = createToken.createTokens(
      user._id as string
    );
...
export const forgotPassword = async (
  ...
  try {
    ...
    await limitResetRequest(user.email, req.ip as string, 3, 60); // 3 lần/phút
